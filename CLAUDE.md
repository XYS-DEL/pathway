...
- Update / feedback / contact entries read channel addresses from `AppConfig` only (`UPDATE_CHECK_ENABLED`, `UPDATE_API_URL`, `FEEDBACK_URL`, `CONTACT_EMAIL`, `CONTACT_GITHUB_URL`). Current product behavior: update check enabled against `XYS-DEL/pathway` latest release; 「问题反馈」opens `FEEDBACK_URL` (GitHub Issues); 「联系作者」shows a dialog to mail `CONTACT_EMAIL` or open `CONTACT_GITHUB_URL`. Manual update/network failure toasts `update_check_failed`; empty-channel fallbacks use `update_disabled` / `feedback_disabled` / `contact_no_channel`. Do not scatter URLs into Activities.
