...
行屿是一个 **Android 定位调试工具**，用途是学习 Android 定位框架、调试自己开发的定位相关应用。
**它不是为规避任何平台规则而设计的。**

1. **严禁用于校园运动类（校园跑、打卡）应用、健身类或游戏类应用的作弊。** 行屿不支持、不赞同，
   也不协助任何形式的此类作弊。
2. **仅供学习 Android 开发与定位调试技术使用。** 请仅在你自己的设备上、针对你自己的应用，或在你
   已获得明确授权的测试场景中使用。
3. **滥用后果由使用者自行承担。** 使用本软件可能违反目标应用的服务条款、你所在学校或单位的规章
   制度，在特定情形下也可能触及当地法律法规。由此产生的一切后果——包括但不限于账号封禁、纪律处分
   与法律责任——由使用者自负，开发者不承担任何责任。
4. **模拟位置是可被检测的。** 本软件依赖 Android 的调试定位 API，需要开启「开发者选项」，而目标
   应用可以直接查询 `Location.isMock()`（API 31+）或 `Location.isFromMockProvider()` 来判断
   位置是否来自模拟提供器。**行屿不做、也不会做任何针对此类检测的规避。**
5. **请勿用于欺骗他人或任何欺诈目的。**
6. **本软件按「现状」提供，不附带任何明示或默示的担保**，包括对适销性、特定用途适用性与不侵权的
   担保；详见 [GPL-3.0](./LICENSE) 第 15、16 条。

继续使用本软件即表示你已阅读、理解并接受上述全部条款。
